...
import io.tokend.template.extensions.highlightLast
...
class LimitProgressWrapper(
    ...
    fun displayProgress(entry: LimitEntry, asset: String) {
        setProgress(
            rootView.daily_limit, entry.statistics.daily,
            entry.limit.daily, asset, R.string.limit_period_daily
        )

        setProgress(
            rootView.weekly_limit, entry.statistics.weekly,
            entry.limit.weekly, asset, R.string.limit_period_weekly
        )

        setProgress(
            rootView.monthly_limit, entry.statistics.monthly,
            entry.limit.monthly, asset, R.string.limit_period_monthly
        )

        setProgress(
            rootView.annual_limit, entry.statistics.annual,
    ...
    private fun setProgress(
        ...
        val left = amountFormatter.formatAssetAmount(
            total - used, simpleAsset,
            abbreviation = true, withAssetCode = false
        )
        val max = amountFormatter.formatAssetAmount(
            total, simpleAsset,
