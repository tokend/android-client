...
class DefaultParticipantEffectConverter : ParticipantEffectConverter {
    override fun toBalanceChanges(participantEffects: Collection<ParticipantsEffectResource>)
        ...
        participantEffects.forEach { participantEffect ->
                ...
                is EffectMatchedResource -> {
                    when (balanceId) {
                        ...
                        effect.funded.balanceAddress ->
                            ...
                            logError(
                                "Cannot choose 'funded' or 'charged' " +
            ...
            if (assetCode == null) {
                logError(
                    "Failed to specify asset of participant effect ${participantEffect.id} " +
        ...
        return try {
            when (operationDetails) {
                ...
                is ManageOfferOpResource ->
                    when (effect) {
                        is EffectMatchedResource -> BalanceChangeCause.MatchedOffer(
                            operationDetails,
                            effect
                        )
            ...
            logError(
                "Unable to parse operation details ${operationDetails.id}: "
                        + e.localizedMessage
            )
            BalanceChangeCause.Unknown
        }
