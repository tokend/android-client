package io.tokend.template.features.history.view
...
class BalanceChangeIconFactory(context: Context) {
    ...
    private val unlockedIcon: Drawable? =
        ContextCompat.getDrawable(context, R.drawable.ic_tx_unlocked)

    fun get(
        action: BalanceChangeListItem.Action,
        isReceived: Boolean?
    ): Drawable? {
        return when (action) {
